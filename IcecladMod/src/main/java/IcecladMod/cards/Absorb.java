...
import IcecladMod.actions.AbsorbAction;
...
public class Absorb extends CustomCard {
    ...
    public static final String ID = IcecladMod.makeID(Absorb.class.getSimpleName());
    private static final CardStrings cardStrings = CardCrawlGame.languagePack.getCardStrings(ID);

    public static final String IMG = makeCardPath("AbsorbPlaceholder.png");
    ...
    private static final float CURRENT_HP_RATIO = 0.5f;
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        AbstractDungeon.actionManager.addToBottom((AbstractGameAction)new VFXAction((AbstractGameEffect)new BiteEffect(m.hb.cX, m.hb.cY - 40.0F * Settings.scale, Color.BLUE.cpy()), 0.3F));
        AbstractDungeon.actionManager.addToBottom( // The action managed queues all the actions a card should do.
                new AbsorbAction(m, new DamageInfo(p, damage, damageTypeForTurn),
                        magicNumber));
    }
    ...
    public void calculateCardDamage(AbstractMonster m) {
        ...
        this.baseDamage += (AbstractDungeon.player.maxHealth - AbstractDungeon.player.currentHealth)*CURRENT_HP_RATIO;
    ...
    public void upgrade() {
        if (!upgraded) {
            upgradeName();
            upgradeMagicNumber(HEAL_AMOUNT_UPGRADE);
